...
export default function ScanManagement() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#101010] text-white space-y-6">
            ...
            {vulns.length === 0 ? (
              ...
              <div key={i} onClick={() => router.push(`/dashboard/developer/scan_management/${vuln.scanId}`)}
                ...
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${severityClass(vuln.severity?.toLowerCase())}`}>
                    {vuln.severity}
                  </span>
                </div>
              </div>
      ...
      <UpgradePlanModal open={showUpgrade} onClose={() => setShowUpgrade(false)} role="developer" />
